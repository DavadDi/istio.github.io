...
手动验证配置是否正确，必要时参考 [Istio API reference](https://istio.io/docs/reference/config)。
...
`x509: certificate signed by unknown authority` 相关的错误通常由 webhook 配置中 `caBundle` 为空引起。验证它是否为空（请参阅[验证 webhook 配置](#invalid-configuration-is-accepted))。`istio-galley` 部署可动态使用保存在 `istio-galley-configuration` `configmap` 中的  webhook 配置和根证书会从 `istio-system` 命名空间中保存的  `istio.istio-galley-service-account` 密钥中加载。
